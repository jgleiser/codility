...
*/
/*
All the following are equal, bit operator is faster
A[i] % Math.pow(2, B[i]);
A[i] & (Math.pow(2, B[i]) - 1);
A[i] & ((1 << B[i]) - 1);
Then to get the (int) of a (float) the fastest is
(int) = (float) | 0
*/
// long version
function solution(A, B) {
...
function solution(A, B) {
    ...
    for (let i = 2; i < A.length + 2; i++) {
        fib[i] = (fib[i-2] + fib[i-1]) & maxMod;
    }
    
    for (let i = 0; i < A.length; i++) {
        result[i] = fib[A[i]+1] & (((1 << B[i]) - 1) | 0);
    }
